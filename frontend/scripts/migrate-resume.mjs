...
const migration = (
  await readFile(
    new URL("../migrations/001_resume_workspace.sql", import.meta.url),
    "utf8",
  )
)
  .replace(/^BEGIN;\s*/, "")
...
try {
  ...
  console.log("Resume migration applied.");
} catch (error) {
  if (error === rollback)
    console.log("Resume migration validated; all changes rolled back.");
  else {
    console.error(
